...
class BaseSingleAgentAviary(BaseAviary):
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            obs = self._getDroneStateVector(0) #self._clipAndNormalizeState(self._getDroneStateVector(0))
            ############################################################
            #### OBS OF SIZE 20 (WITH QUATERNION AND RPMS)
            # return obs
            ############################################################
            #### OBS SPACE OF SIZE 12
            return np.hstack([obs[0:3], obs[7:10], obs[10:13], obs[13:20]]).reshape(16,)
            ############################################################
        else:
